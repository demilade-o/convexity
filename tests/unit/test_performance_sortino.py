...
class TestHandWorkedDerivation:
    def test_matches_hand_derivation_exactly(self, hand_worked_returns: pd.Series):
...
class TestDownsideConvention:
    ...
    def test_default_is_full(self, hand_worked_returns: pd.Series):
    ...
    def test_sortino_is_lower_under_downside_only(self, hand_worked_returns: pd.Series):
...
class TestScalarTarget:
    ...
    def test_higher_mar_lowers_the_ratio(self, hand_worked_returns: pd.Series):
...
class TestTimeVaryingTarget:
    ...
    def test_sparse_rate_series_is_carried_forward_as_of(
        ...
        result = cx.sortino_ratio(
            hand_worked_returns, mar=quotes, periods_per_year=12
        )
    ...
    def test_future_rate_does_not_leak_backwards(self, hand_worked_returns: pd.Series):
...
class TestZeroDenominatorPolicy:
    ...
    def test_no_downside_and_zero_excess_is_nan(self, monthly_index):
    ...
    def test_all_below_target_is_negative_and_finite(self, monthly_index):
...
class TestFrequency:
    ...
    def test_annualisation_scales_by_sqrt_m(self, hand_worked_returns: pd.Series):
    ...
    def test_no_annualisation_route_raises_rather_than_assuming_252(self):
    ...
    def test_inference_from_index_is_used_when_confident(self):
...
class TestMissingData:
    def test_nan_raises_by_default(self, monthly_index):
    ...
    def test_drop_policy_computes_on_remaining_observations(self, monthly_index):
        returns = pd.Series([0.01] * 11 + [float("nan")], index=monthly_index)
        result = cx.sortino_ratio(
    ...
    def test_drop_matches_manually_cleaned_series(self, monthly_index):
        with_gap = pd.Series(
        ...
        assert cx.sortino_ratio(
            ...
            cx.sortino_ratio(cleaned, periods_per_year=12), rel=1e-12
        )

    def test_infinite_return_is_always_rejected(self, monthly_index):
        returns = pd.Series([0.01] * 11 + [float("inf")], index=monthly_index)
        with pytest.raises(DataQualityError, match="infinite"):
            cx.sortino_ratio(returns, periods_per_year=12)

    def test_return_below_minus_one_is_rejected(self, monthly_index):
        returns = pd.Series([0.01] * 11 + [-1.5], index=monthly_index)
        with pytest.raises(DataQualityError, match="below -1.0"):
            cx.sortino_ratio(returns, periods_per_year=12)

    def test_total_loss_is_permitted(self, monthly_index):
        returns = pd.Series([0.01] * 11 + [-1.0], index=monthly_index)
        assert math.isfinite(cx.sortino_ratio(returns, periods_per_year=12))


class TestRollingWindows:
    def test_rolling_length_matches_input(self, mixed_returns: pd.Series):
    ...
    def test_incomplete_windows_are_nan_not_fabricated(self, mixed_returns: pd.Series):
        result = cx.rolling_sortino(mixed_returns, window=6, periods_per_year=12)
    ...
    def test_full_window_equals_static_ratio(self, mixed_returns: pd.Series):
        n = len(mixed_returns)
    ...
    def test_rolling_accepts_time_varying_target(self, mixed_returns: pd.Series):
...
class TestNonMutation:
    def test_input_series_is_not_modified(self, hand_worked_returns: pd.Series):
    ...
    def test_target_series_is_not_modified(self, hand_worked_returns: pd.Series):
        quotes = pd.Series([0.05], index=pd.to_datetime(["2023-12-01"]))
